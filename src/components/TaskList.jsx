...
export default function TaskList({ activePage, filter, onEditTask, darkMode }) {
  ...
  const getPriorityColor = (priority) => {
    switch (priority?.toLowerCase()) {
      case 'high': return darkMode ? 'text-red-300 bg-red-900/50' : 'text-red-600 bg-red-50';
      case 'medium': return darkMode ? 'text-yellow-300 bg-yellow-900/50' : 'text-yellow-600 bg-yellow-50';
      case 'low': return darkMode ? 'text-green-300 bg-green-900/50' : 'text-green-600 bg-green-50';
      default: return darkMode ? 'text-gray-300 bg-gray-700' : 'text-gray-600 bg-gray-50';
    }
  };

  const getStatusColor = (status) => {
    switch (status?.toLowerCase()) {
      case 'completed': return darkMode ? 'text-green-300 bg-green-900/50' : 'text-green-600 bg-green-50';
      case 'in-progress': return darkMode ? 'text-blue-300 bg-blue-900/50' : 'text-blue-600 bg-blue-50';
      case 'todo': return darkMode ? 'text-gray-300 bg-gray-700' : 'text-gray-600 bg-gray-50';
      case 'on-hold': return darkMode ? 'text-orange-300 bg-orange-900/50' : 'text-orange-600 bg-orange-50';
      default: return darkMode ? 'text-gray-300 bg-gray-700' : 'text-gray-600 bg-gray-50';
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className={`flex justify-between items-center ${darkMode ? 'bg-gray-800' : 'bg-white'} rounded-lg p-4 shadow-sm`}>
        <div>
      ...
      {filteredTasks.length === 0 ? (
        ...
        <div className={`text-center py-12 ${darkMode ? 'bg-gray-800' : 'bg-white'} rounded-lg`}>
          <TaskIcon className={`mx-auto ${darkMode ? 'text-gray-500' : 'text-gray-400'} mb-4`} style={{ fontSize: 64 }} />
          <h3 className={`text-lg font-medium ${darkMode ? 'text-gray-200' : 'text-gray-600'} mb-2`}>No tasks found</h3>
              ...
              <div className="flex items-start justify-between gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-3 mb-2">
                    <h3 className={`font-semibold ${darkMode ? 'text-white' : 'text-gray-800'} truncate`}>{task.title}</h3>
                  ...
                  {task.description && (
                    <p className={`text-sm ${darkMode ? 'text-gray-300' : 'text-gray-600'} mb-3 line-clamp-2`}>
                      {task.description}
                    </p>
                  )}
          ...
          >
            <table className="w-full table-fixed min-w-[1300px] border-collapse">
              ...
              <tbody className={`${darkMode ? 'bg-gray-800 divide-gray-700' : 'bg-white divide-gray-200'} divide-y`}>
                {filteredTasks.map((task) => (
                  <tr key={task.id} className={`${darkMode ? 'hover:bg-gray-700' : 'hover:bg-gray-50'} border-b ${darkMode ? 'border-gray-700' : 'border-gray-200'}`}>
                    <td className={`px-4 py-4 w-64 border-r ${darkMode ? 'border-gray-700' : 'border-gray-200'}`}>
                      <div className="flex flex-col">
                        <div className={`text-sm font-medium ${darkMode ? 'text-white' : 'text-gray-900'} truncate`}>{task.title}</div>
                        {task.description && (
                        ...
                        {task.tags && task.tags.length > 0 && (
                          <div className="flex flex-wrap gap-1 mt-1 max-w-full">
                            ...
                            {task.tags.length > 2 && (
                              <span className={`text-xs ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>+{task.tags.length - 2}</span>
                            )}
                          </div>
                        )}
                      </div>
                    ...
                    <td className={`px-4 py-4 w-24 border-r ${darkMode ? 'border-gray-700' : 'border-gray-200'}`}>
                      <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${getPriorityColor(task.priority)} whitespace-nowrap`}>
                        {task.priority || 'medium'}
                      </span>
                    </td>
                    <td className={`px-4 py-4 w-28 border-r ${darkMode ? 'border-gray-700' : 'border-gray-200'}`}>
                      <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${getStatusColor(task.status)} whitespace-nowrap`}>
                        {task.status || 'todo'}
                      </span>
                    </td>
                    {/* <td className={`px-4 py-4 text-sm ${darkMode ? 'text-gray-300' : 'text-gray-900'} w-32 truncate border-r ${darkMode ? 'border-gray-700' : 'border-gray-200'}`}>
