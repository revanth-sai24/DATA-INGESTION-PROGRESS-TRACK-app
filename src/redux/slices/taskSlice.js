  ...
  try {
    ...
    if (response.ok) {
      console.log(
        ...
        "projects to sample-projects.csv",
      );
    } else {
      const errorText = await response.text();
      ...
      Papa.parse(file, {
        ...
        complete: (results) => {
          const validRows = results.data.filter(
            (row) => row.title && row.title.trim() && row.id !== "PROJECT",
          );
  ...
  reducers: {
    addTask: (state, action) => {
      const newTask = {
        ...
        colorLabel: action.payload.colorLabel || null,
      };
    ...
    restoreTask: (state, action) => {
      const taskToRestore = state.archivedTasks.find(
        (task) => task.id === action.payload,
      );
      if (taskToRestore) {
    ...
    addProject: (state, action) => {
      ...
      const projectName =
        typeof newProject === "string" ? newProject : newProject.name;
      const existingProject = state.projects.find(
        (p) => (typeof p === "string" ? p : p.name) === projectName,
      );
    ...
    // New actions for Kanban, Favorites, and Color Labels
    duplicateTask: (state, action) => {
      ...
      if (taskToDuplicate) {
        const duplicatedTask = {
          ...
          timeTracking: { elapsed: 0, isRunning: false, startTime: null },
          checkpoints:
            taskToDuplicate.checkpoints?.map((cp) => ({
              ...cp,
    ...
    setColorLabel: (state, action) => {
      const { taskId, colorLabel } = action.payload;
    ...
    builder
      ...
      .addCase(importFromCSV.fulfilled, (state, action) => {
        ...
        const importedProjects = [
          ...new Set(
            action.payload.map((task) => task.project).filter(Boolean),
          ),
        ];
        const allProjects = [
        ...
        state.tasks = action.payload.map((task) => ({
          ...
          tags: Array.isArray(task.tags)
            ...
            : typeof task.tags === "string" && task.tags
              ? task.tags.split(";").filter((t) => t.trim())
              : [],
...
export const selectPinnedTasks = (state) =>
  state.tasks.tasks.filter((t) => t.pinned && t.status !== "archived");
...
export const selectFilteredTasks = createSelector(
  ...
  (tasks, filter, searchQuery, sortConfig) => {
    let filtered = tasks.filter((task) => {
      ...
      return (
        matchesStatus && matchesProject && matchesPriority && matchesSearch
      );
    });
